...
>  where 	
>    fibonOutput act a = if iters == 1 then act else deepseq a (return ())
>    runParserGen cli fl_name =
...
>	   (fibonOutput (hPutStrLn stderr ("unused rules: " ++ show (length unused_rules))) unused_rules) >>
>	optIO (not (null unused_terminals))
>	   (fibonOutput (hPutStrLn stderr ("unused terminals: " ++ show (length unused_terminals)) )unused_terminals) >>
...
>	    (if sr /= 0
>		then fibonOutput (hPutStrLn stderr ("shift/reduce conflicts:  " ++ show sr)) rr
>		else return ())

>	    (if rr /= 0
>		then fibonOutput (hPutStrLn stderr ("reduce/reduce conflicts: " ++ show rr)) rr
>		else return ())
